...
### Fixed
    ...
    - query key on `useFetchProposals`
    - variable name in `onProposalTransaction?: (txHash: string) => void;` in the `useNewProposal`
    - export `ProposalStatus`
    - update the config type to allow any headers
