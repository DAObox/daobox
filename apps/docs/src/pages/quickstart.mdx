...
<Note>
  The quickest way to get started with use-aragon is to use the [use-aragon]()
  boilerplate.
</Note>
...
```bash {{ title: 'npm' }}
npm install @daobox/use-aragon
```

```bash {{ title: 'pnpm' }}
pnpm install @daobox/use-aragon
```

```bash {{ title: 'yarn' }}
yarn add @daobox/use-aragon
```
...
- [Checkout the guides section ](#)
- [Jump straight in into the hooks ](#)
- [Come and talk to us in discord](#)
